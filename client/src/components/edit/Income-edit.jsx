...
export default function IncomeEdit({
  ...
  const [value, setValue] = useState(incomeDetailed?.value || 0);
  const [datew, setDatew] = useState(incomeDetailed?.date?.slice(0, 10) || "");
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await ConsumptionApi.updateIncome(
        incomeDetailed._id,
        updatedData
      );
      setIncomeDetailed(updatedData);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center p-4 bg-black/90 text-white">
      ...
      >
        <h2 className="font-bold text-xl">Редактировать приход</h2>
        ...
        <div className="w-full flex flex-col justify-center items-center">
          ...
          <div className="flex w-10/12 gap-4">
            ...
            <button
              type="submit"
              className="w-10/12 bg-green-600 text-white rounded h-8 mt-6 hover:bg-gray-200 transition-colors"
            >
              Сохранить
            </button>
          </div>
